...
class APIService {
  ...
  // Get all quizzes - NOW USES BACKEND ENDPOINT
  async getAllQuizzes() {
    try {
      return await this.request("quiz", "/api/quizzes");
    } catch (error) {
  ...
  async submitQuizAttempt(quizId, studentId, answers) {
    try {
      ...
      return await this.request(
        "quiz",
        `/api/quizzes/${quizId}/submit/${studentId}`,
        {
          method: "POST",
  ...
  // Get all attempts - NOW USES BACKEND ENDPOINT
  async getAllAttempts() {
    try {
      return await this.request("quiz", "/api/quizzes/attempts");
    } catch (error) {
  ...
  async reviewSubmission(id, marks, feedback) {
    try {
      ...
      return await this.request(
        "submission",
        `/api/submissions/${id}/review?${params.toString()}`,
        {
          method: "PUT",
        }
      );
  ...
  async getDashboardStats() {
    try {
      const [students, lecturers, courses, submissions, quizzes, attempts] =
        await Promise.all([
          this.getStudents(0, 1).catch(() => ({
            totalElements: 0,
            content: [],
          })),
